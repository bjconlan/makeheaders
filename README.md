...
- &#8220;<code>extern</code>&#8221; variable declarations.
...
1. In large codes with many source files, it becomes difficult to determine
   which .h files should be included in which .c files.
2. It is typically the case that a .h file will be forced to include
...
3. Some information is common to both the declaration and the definition of
   ...
   files in sync.
4. When a .c file includes a .h file and the .h files changes, the .c file
...
5. The C programming language requires that declarations depending upon
...
1. Because all declarations needed by a .c file are contained in a
...
5. The makeheaders program automatically sorts declarations into the
...
- When a function is defined in any .c file, a prototype of that function
  ...
  If the &#8220;<code>static</code>&#8221; keyword of C appears at the
  ...
  When the &#8220;<code>LOCAL</code>&#8221; keyword is used, makeheaders will
  ...
  If you invoke makeheaders with a &#8220;<code>-local</code>&#8221;
...
- Global variables
- Preprocessor macros (ex: &#8220;<code>#define</code>&#8221;)
